...
namespace QuantConnect.Tests.Common.Securities
{
    ...
    public class SecurityMarginModelTests
    {
        ...
        [Test]
        public void GetMaximumOrderQuantityForTargetDeltaBuyingPower_NoHoldings_Positive()
        {
            var algo = GetAlgorithm();
            var security = InitAndGetSecurity(algo, 5);

            var buyingPower = algo.Portfolio.MarginRemaining;
            var actual = security.BuyingPowerModel.GetMaximumOrderQuantityForTargetDeltaBuyingPower(
                new GetMaximumOrderQuantityForTargetValueParameters(algo.Portfolio,
                    security,
                    buyingPower)).Quantity;

            // (100000 * - 2.0m * 0.9975 setHoldingsBuffer) / 25 - fee~=-7999m
            Assert.AreEqual(-7999m, actual);
            Assert.IsTrue(HasSufficientBuyingPowerForOrder(actual, security, algo));
        }

        [Test]
        public void GetMaximumOrderQuantityForTargetDeltaBuyingPower_NoHoldings_Negative()
        {
            var algo = GetAlgorithm();
            var security = InitAndGetSecurity(algo, 5);
            ...
            var actual = security.BuyingPowerModel.GetMaximumOrderQuantityForTargetDeltaBuyingPower(
                new GetMaximumOrderQuantityForTargetValueParameters(algo.Portfolio,
                    security,
                    buyingPower)).Quantity;
            // we want to decrease our current used buying power but we hold no position so its 0
        ...
        [TestCase(100, -510)]
        [TestCase(-100, -510)]
        public void GetMaximumOrderQuantityForTargetDeltaBuyingPower_WithHoldings(decimal quantity, decimal buyingPowerDelta)
        {
            var algo = GetAlgorithm();
            var security = InitAndGetSecurity(algo, 0);
